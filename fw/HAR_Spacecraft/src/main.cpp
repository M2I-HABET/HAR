...
void setup() {
  ...
  if (!SD.exists("/HARdata.csv")) {
    File file = SD.open("/HARdata.csv", FILE_WRITE);
    if (file) {
      file.println("Time,Latitude,Longitude,Altitude_mm,Heading_degE5,Speed_mmps,PDOP,Pressure_Pa,Temperature_cC,Humidity_rh%,FixType,CheckStatus,PacketCounter,BatteryVoltage_V");
      file.close();
      Serial.println("CSV header written.");
    } else {
      Serial.println("Failed to create HARdata.csv");
    }
  } else {
    Serial.println("HARdata.csv already exists. Skipping header write.");
  }
...
void loop() {
 // Receive
  ...
  if (file) {
    ...
    file.print(counter); 
    file.print(",");
    file.println(volt, 2);  // two decimal places
    file.close();
  } else {
    Serial.println("Failed to open HARdata.csv for appending.");
  }
